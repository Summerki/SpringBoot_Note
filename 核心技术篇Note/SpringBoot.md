...
![1566302351714](images/1566302351714.png)
...
![1566306669721](images/1566306669721.png)

![1566306744492](images/1566306744492.png)

![1566306999904](images/1566306999904.png)

![1566307028516](images/1566307028516.png)

![1566349710324](images/1566349710324.png)
...
想让Spring的配置文件生效，加载进来；@**ImportResource**标注在一个配置类上（一般在主配置类上注解）
...
![1566526232307](images/1566526232307.png)
...
![1566524755050](images/1566524755050.png)
...
![1566528935446](images/1566528935446.png)

![1566528957963](images/1566528957963.png)
...
![1566535307374](images/1566535307374.png)
...
![1566543900771](images/1566543900771.png)
...
ServerProperties也是定制器EmbeddedServletContainerCustomizer
```
...
3）、后置处理器，从容器中获取所有的**EmbeddedServletContainerCustomizer**（嵌入式Servlet容器定制其），调用定制器的定制方法
...
**先启动嵌入式的Servlet容器，再将ioc容器中其他剩下没有创建出的对象获取出来；**
...
![1566609491186](images/1566609491186.png)

创建web resources目录的方法：

![1566609986055](images/1566609986055.png)

![1566610050544](images/1566610050544.png)

![1566610090976](images/1566610090976.png)

![1566610169542](images/1566610169542.png)

![1566610216006](images/1566610216006.png)
...
servlet3.0（Spring注解版）里面的8.2.4章节（Shared libraries / runtimes pluggability）有如下描述：
...
![1566611328665](images/1566611328665.png)
...
​		桥接网络=>选好网卡=>接入网线；（保持虚拟机和本机IP在同一网段内）
...
但上面的正确的启动还是用不了！！！因为没有做端口映射！！！做了端口映射看下面：
...
![1566614400288](images/1566614400288.png)
...
spring:
  datasource:
    username: sumisql
    password: 123456
    url: jdbc:mysql://192.168.1.1:3306/jdbc
...
+ 默认是用`com.zaxxer.hikari.HikariDataSource`作为数据源；
  + 注：spring boot1.X是tomcat数据源；spring boot2.X是hikari数据源
...
![1566615616682](images/1566615616682.png) 

注意：执行指定位置的sql文件spring boot需要加上`initialization-mode: always`

![1566617398701](images/1566617398701.png)

注意：出现异常`The server time zone value 'ÖÐ¹ú±ê×¼Ê±¼ä' is unrecognized or represents more than one time zone`的解决办法：
...
![1566618116127](images/1566618116127.png)
...
![1566619382710](images/1566619382710.png)
...
启动后访问`http://localhost:8080/druid`登陆即可看到Druid后台了



## 3、整合MyBatis

![1566619955554](images/1566619955554.png)

```xml
<!--这个看下面就知道不是spring boot官方出的starter，是mybatis自己给spring boot做的starter-->
<dependency>
...
public interface DepartmentMapper {
    ...
    @Options(useGeneratedKeys = true, keyProperty = "id")  // 指明主键是谁
...
问题：假如数据库里的字段是`department_name`如何与java bean中的`departmentName`对应呢？

解决办法：自定义MyBatis的配置规则；给容器中添加一个ConfigurationCustomizer；
...
如果你有很多mapper文件，在每一个mapper文件都加上`@Mapper`注解会显得很麻烦，可以在启动spring boot主类上加上`@MapperScan`注解解决，如下：
...
![1566632028151](images/1566632028151.png)

![1566632119786](images/1566632119786.png)
...
![1566632683313](images/1566632683313.png)

# 注：后面两节的内容很重要，还没怎么听懂，以后还要看
